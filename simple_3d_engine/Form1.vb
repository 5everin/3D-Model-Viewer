...
Public Class Form1
  ...
  Private Sub Nu_rasterPoly(ByVal vrt() As Vector3, ByVal ply() As Poly)
    ...
    Parallel.For(0, polycount, Sub(f As Int32)
                                 ...
                                 If CheckBox1.Checked Then
                                   ...
                                   If CheckBox7.Checked = False AndAlso CheckBox4.Checked = True Then tmpc = colour
  ...
  Public Sub FillsnCols(indx As Int32, vec As Vector3, ByRef colour As Int32)
    ...
    If CheckBox10.Checked OrElse Lit_lines = False Then
      Dim Rcolour As Int32 = Floor(Vector3.Distance(modelcenter, vec * 0.05)) Mod 255
    ...
    If DrawAline AndAlso Lit_lines = False Then
  ...
  Private Sub DrawX(ByVal l1 As Vector3, ByVal l2 As Vector3, ByVal colour As Int32) 'draws a line along the x axis generating z axis coordinates for the zbuffer. (with or without alpha blending)
    ...
    For n As Int32 = l1.X + 1 To l2.X
      If n >= 0 AndAlso n < Swidth Then
        loc = n + (l2.Y * Swidth)
        If loc < size_array AndAlso loc >= 0 Then
          If CheckBox12.Checked AndAlso pre_post = False Then  'alpha blending (ignores the zbuffer - dirty hack)
            Dim rb1 As Int32 = colour And &HFF00FF
            Dim g1 As Int32 = colour And &HFF00
            rb1 += ((Bigarray(loc) And &HFF00FF) - rb1) * blendamount >> 8
            g1 += ((Bigarray(loc) And &HFF00) - g1) * blendamount >> 8
